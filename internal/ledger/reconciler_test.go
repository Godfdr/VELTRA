...
func TestConcurrency_ZeroRaceDeadlocks(t *testing.T) {
	...
	if err != nil {
		t.Skip("Postgres container offline; spin up via docker-compose to execute chaos tests")
	}
	defer pool.Close()
	...
	for i := 1; i <= concurrentWorkers; i++ {
		...
		txRef := "chaos_tx_ref_" + time.Now().Format("150405.000000") + string(rune(workerCounter))
	...
	t.Logf("Chaos test finalized. Remaining Sender Wallet Units: %d minor units.", finalSenderBalance)
}
...
func TestCounterReplayAttack_Rejection(t *testing.T) {
	// Logic to check counter sequence regression rejection
}
